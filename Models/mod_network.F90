module mod_network
  ...
  type :: network_type
    ...
    real(KDR) :: lr
    integer(KDI) :: num_dense_layers, input_size, output_size
    real(KDR), allocatable :: layer_info(:)
...
contains
  ...
  real(KDR) function accuracy(self, x, y)
  ...
  subroutine init(self, layer_names, layer_info)
    ...
    class(network_type), intent(in out) :: self
    real(KDR), intent(in) :: layer_info(:)
    integer(KDI), allocatable :: dense_dims(:)
    integer(KDI) :: n, i, dense_idx, unique_layers
    character(len=100), intent(in) :: layer_names(:)
  ...
  subroutine load(self, filename)
    ...
    integer(KDI) :: fileunit, n, num_layers, end_of_file
    real(KDR), allocatable :: layer_info(:)
  ...
  real(KDR) function loss(self, y_true, y_pred)
    ! Given input x and expected output y, returns the loss of the network.
    class(network_type), intent(in out) :: self
    real(KDR), intent(in) :: y_true(:), y_pred(:)
  ...
  function output(self, input) result(a)
    ...
    class(network_type), intent(in out) :: self
    real(KDR), intent(in) :: input(:)
    real(KDR), allocatable :: a(:)
    integer(KDI) :: n
